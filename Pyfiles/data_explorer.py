...
def preprocess() :
    ...
    for col in score_columns : 
        fig = px.box(mental, y=col, color_discrete_sequence=px.colors.sequential.solar) # Purple color scheme for boxplots  
        fig.show()
    ...
    for col in mental_scaled.columns: 
        if mental_scaled[col].dtype != 'object':
            fig = px.box(mental_scaled, y=col, color_discrete_sequence=px.colors.sequential.solar) # Purple color scheme for boxplots
            print("=" * 80)
    ...
    for col in mental_scaled.columns :
        if mental_scaled[col].dtype != 'object' :
            fig = px.histogram(mental_scaled, x=col, nbins=30, color_discrete_sequence=px.colors.sequential.solar) 
            print("=" * 80)
            print(f"Variable : {col}")
            print("=" * 80)
            fig.show()
...
# To run the preprocessing function from another script, you can import it and call it as follows:
# if __name__ == "__main__": # Ca permet de s'assurer que le code ne s'exécute que lorsque le script est exécuté directement, et pas lorsqu'il est importé en tant que module dans un autre script.
    # preprocess()
# or  from data_explorer import preprocess # preprocess()
...
def prepare_data_for_ml():
    ...
    # Handle Gender by replacing 3 with 1
    mental_scaled['Gender'] = mental_scaled['Gender'].replace(3, 1) # Assuming 1 = Male, 2 = Female
    # Create target variable for burnout risk based on established cutoffs
    mental_scaled['Burnout_Risk'] = ((mental_scaled['Burnout_Emotional_Exhaustion_Score'] >= 27) | (mental_scaled['Burnout_Cynicism_Score'] >= 13) | (mental_scaled['Academic_Efficacy_Score'] <= 31)).astype(int)
    # Define features and target variable
    features = mental_scaled.drop(columns=['Burnout_Risk', 'Languages_Spoken'])
    target = mental_scaled['Burnout_Risk']
